# REGULR.vip LOYALTY PLATFORM - THE MASTER BLUEPRINT (v10)
...
### Phase 7: Gated Onboarding (KYC-light) — ✅ COMPLEET (100%)
- [x] Database migration: `account_status` + verification columns on `users`, rate limit columns on `tenants`, `verification_attempts` audit table (`sql/gated_onboarding_migration.sql`).
- [x] Backward compatibility: existing guests with `balance > 0` auto-set to `account_status = 'active'`.
- [x] User model: 8 new methods for account status, verification, suspension, rate limiting (`models/User.php`).
- [x] AuthService: suspended login block + `account_status` in session info (`services/AuthService.php`).
- [x] WalletService: account_status guard in `createDeposit()` — unverified cannot deposit (`services/WalletService.php`).
- [x] PaymentService: account_status guard in `processPayment()` — suspended/unverified cannot pay (`services/PaymentService.php`).
- [x] API: Bartender verification endpoint with birthdate matching + rate limiting (`api/pos/verify.php`).
- [x] API: Admin suspend/unsuspend endpoint with reason + push notification (`api/admin/suspend_user.php`).
- [x] API: `account_status` added to QR scan response (`api/pos/scan.php`).
- [x] API: Verification limit fields in settings GET/POST (`api/admin/settings.php`).
- [x] Router: `/api/pos/verify` + `/api/admin/suspend_user` routes registered (`index.php`).
- [x] Config: Platform minimum constants for verification limits (`config/app.php`).
- [x] Bartender POS: New `#state-verify` panel with birthdate input + verify/activate flow (`views/bartender/dashboard.php`).
- [x] Guest dashboard: Account status banner + disabled "Opwaarderen" for unverified (`views/guest/dashboard.php`).
- [x] Guest wallet: Deposit gating + verification banner with QR link (`views/guest/wallet.php`).
- [x] Admin settings: "Verificatie Limieten" section with 4 configurable rate limit inputs (`views/admin/settings.php`).
- [x] Admin users: Status column with color-coded badges (`views/admin/users.php`).
- [x] Push notifications on verification success + account suspension.
- [x] Plan document: `.kilo/plans/gated_onboarding_implementation.md`.
